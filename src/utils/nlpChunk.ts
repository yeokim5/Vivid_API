...
function distributeContentUnits(
  ...
  for (let i = 0; i < sectionCount; i++) {
    ...
    const sectionUnits = units.slice(
      currentIndex,
      Math.min(currentIndex + unitsForThisSection, totalUnits)
    );
 ...
 */
function distributeTextByCharacters(
  text: string,
  sectionCount: number
): Record<string, string> {
  ...
  for (let i = 0; i < sectionCount; i++) {
    ...
    const endPos =
      i === sectionCount - 1 ? totalLength : (i + 1) * charsPerSection;
