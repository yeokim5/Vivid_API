...
class QueueManager {
  ...
  private lastProcessedTime: number = Date.now() - 68000; // Initialize to allow immediate first processing
  private isProcessing: boolean = false;
  private currentProcessingUserId: string | null = null;
  private currentProcessingItemId: string | null = null; // Track specific item being processed
  private readonly PROCESSING_INTERVAL = 68000; // 1 minute + 8 seconds buffer in milliseconds
   ...
   */
  addToQueue(item: QueueItem): QueueStatus {
    ...
    if (existingIndex !== -1) {
      ...
      console.log(
        `[QUEUE] Updated existing queue item ${item.id} for user ${item.userId}`
      );
    ...
    console.log(
      `[QUEUE] User ${item.userId} added to queue with ID ${item.id}. Position: ${this.queue.length}, Total in queue: ${this.queue.length}`
    );
   ...
   */
  removeFromQueueByItemId(itemId: string): boolean {
    const initialLength = this.queue.length;
  ...
  getQueueStatus(userId: string): QueueStatus {
    ...
    if (position > 0) {
      if (this.isProcessing) {
        ...
        const timeUntilNextProcess = Math.max(
          0,
          this.PROCESSING_INTERVAL - timeSinceLastProcess
        );
        estimatedWaitTime =
    ...
    console.log(
      `[QUEUE] Status for user ${userId}: position=${position}, totalInQueue=${totalInQueue}, isProcessing=${this.isProcessing}, estimatedWait=${estimatedWaitTime}ms`
    );
   ...
   */
  getQueueStatusByItemId(itemId: string): QueueStatus | null {
    ...
    if (this.isProcessing) {
      ...
      estimatedWaitTime = -1; // Special value meaning "waiting for current processing"
      console.log(
        `[QUEUE] Item ${itemId} waiting for current processing to finish.`
      );
    } else {
      // No one is processing - calculate based on rate limit
      const timeSinceLastProcess = Date.now() - this.lastProcessedTime;
    ...
    console.log(
      `[QUEUE] Status for item ${itemId}: position=${position}, totalInQueue=${totalInQueue}, isProcessing=${this.isProcessing}, estimatedWait=${estimatedWaitTime}ms`
    );
  ...
  canProcess(): boolean {
    ...
    const canProcessNow =
      timeSinceLastProcess >= EFFECTIVE_INTERVAL && !this.isProcessing;

    // Add debug logging for timing issues
    if (!canProcessNow && this.queue.length > 0) {
      console.log(
        `[QUEUE] canProcess=false: timeSince=${timeSinceLastProcess}ms, required=${EFFECTIVE_INTERVAL}ms, isProcessing=${this.isProcessing}`
      );
    }
   ...
   */
  getNextForProcessing(): QueueItem | null {
    ...
    if (!this.canProcess()) {
      ...
      const EFFECTIVE_INTERVAL = this.PROCESSING_INTERVAL - 1000; // 67 seconds
      console.log(
        `[QUEUE] getNextForProcessing: canProcess=false, timeSince=${timeSinceLastProcess}ms, required=${EFFECTIVE_INTERVAL}ms`
      );
      return null;
    }

    console.log(
      `[QUEUE] getNextForProcessing: Returning item ${this.queue[0].id} for user ${this.queue[0].userId}`
    );
   ...
   */
  tryStartImmediateProcessing(userId: string): boolean {
  ...
  startProcessing(userId: string): boolean {
    ...
    if (
      ...
      console.log(
        `[QUEUE] Started processing for user ${userId} with item ${item.id} (from queue)`
      );
      return true;
    }
   ...
   */
  completeProcessing(userId: string): boolean {
    if (!this.isProcessing || this.currentProcessingUserId !== userId) {
      console.log(
        `[QUEUE] Complete processing failed: isProcessing=${this.isProcessing}, currentUser=${this.currentProcessingUserId}, requestUser=${userId}`
      );
      return false;
    }
    ...
    console.log(
      `[QUEUE] Setting lastProcessedTime to ${this.lastProcessedTime} for user ${userId}`
    );

    // Case 1: Processing a specific queue item
    if (this.currentProcessingItemId) {
      const itemId = this.currentProcessingItemId;
      const removed = this.removeFromQueueByItemId(
        this.currentProcessingItemId
      );
      this.isProcessing = false;
      this.currentProcessingUserId = null;
      this.currentProcessingItemId = null;
      console.log(
        `[QUEUE] Completed processing for user ${userId} with item ${itemId} (removed: ${removed})`
      );
      return true;
    }
    ...
    this.currentProcessingItemId = null;
    console.log(
      `[QUEUE] Completed processing for user ${userId} (immediate processing)`
    );
    return true;
  }
   ...
   */
  getAllQueueStatuses(): { [userId: string]: QueueStatus } {
    const statuses: { [userId: string]: QueueStatus } = {};
  ...
  private startQueueProcessor(): void {
    setInterval(() => {
      if (this.queue.length > 0 && this.canProcess() && !this.isProcessing) {
        ...
        console.log(
          `[QUEUE] Ready to process next item for user ${nextItem.userId} (item ${nextItem.id}). Queue length: ${this.queue.length}, isProcessing: ${this.isProcessing}`
        );
