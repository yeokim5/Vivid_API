...
export function generateHtmlFromTemplate(
  ...
  templatePath: string = path.join(__dirname, "template.html")
): string {
  try {
    ...
    htmlTemplate = htmlTemplate.replace(
      /\[username\]/gi,
      contentData.username || "Anonymous"
    );

    // Apply custom styling by updating CSS variables
    htmlTemplate = htmlTemplate.replace(
      /\[TEXT_COLOR\]/gi,
      contentData.textColor || "#f8f9fa"
    );
    htmlTemplate = htmlTemplate.replace(
      /\[TITLE_COLOR\]/gi,
      contentData.titleColor || "#cccac4"
    );
    htmlTemplate = htmlTemplate.replace(
      /\[FONT_FAMILY\]/gi,
      contentData.fontFamily || "Playfair Display"
    );

    // Apply box background color and opacity
    const boxBgColor = contentData.boxBgColor || "#585858";
    const boxOpacity =
      contentData.boxOpacity !== undefined ? contentData.boxOpacity : 0.5;
  ...
  background: rgba(${parseInt(boxBgColor.slice(1, 3), 16)}, ${parseInt(
      boxBgColor.slice(3, 5),
      16
    )}, ${parseInt(boxBgColor.slice(5, 7), 16)}, ${boxOpacity});`;
    ...
    htmlTemplate = htmlTemplate.replace(
      /\[Header_background\]/gi,
      contentData.header_background_image || ""
    );
    htmlTemplate = htmlTemplate.replace(
      /\[BG_1\]/gi,
      contentData.section1_image_url || ""
    );
    htmlTemplate = htmlTemplate.replace(
      /\[BG_2\]/gi,
      contentData.section2_image_url || ""
    );
    htmlTemplate = htmlTemplate.replace(
      /\[BG_3\]/gi,
      contentData.section3_image_url || ""
    );
    htmlTemplate = htmlTemplate.replace(
      /\[BG_4\]/gi,
      contentData.section4_image_url || ""
    );
    htmlTemplate = htmlTemplate.replace(
      /\[BG_5\]/gi,
      contentData.section5_image_url || ""
    );
    htmlTemplate = htmlTemplate.replace(
      /\[BG_6\]/gi,
      contentData.section6_image_url || ""
    );
    htmlTemplate = htmlTemplate.replace(
      /\[BG_7\]/gi,
      contentData.section7_image_url || ""
    );
    htmlTemplate = htmlTemplate.replace(
      /\[BG_8\]/gi,
      contentData.section8_image_url || ""
    );
    htmlTemplate = htmlTemplate.replace(
      /\[BG_9\]/gi,
      contentData.section9_image_url || ""
    );

    // Set the background effect
    htmlTemplate = htmlTemplate.replace(
      /\[BACKGROUND_EFFECT\]/gi,
      contentData.backgroundEffect || "none"
    );
